...
export class LivePrinter {
  ...
  */
  constructor(model = "UM2plus") {
    ...
    /**
    * Maps drum name to movement speed to get drum-like sounds
    */
    this.drum2speed = {
      hh: 200,
      cp: 150,
  ...
  */
  ...
  /**
  * Parse an argument (beats, seconds, ms) to time in ms. NOTE: attempts to stay in musical time
  * by adhering to the interval -- for larger intervals, this might mean falling short or long of the target movement time!
  * 
  ...
  */
  async drawtime(time) {
    ...
    while (safetyCounter && (targetTime - this.totalMoveTime > 1e-4)) 
      ...
      const remaining = targetTime - this.totalMoveTime;
      
      // calculate new distance based on this._intervalTime
      const rawDt = this._timeWarp({
  ...
  */
  parseAsNote(note, bpm = this._bpm) {
    ...
    if (isFinite(note)) {
      ...
      else if (/^hh|bd|cp|oh|sd$/.test(noteStr)) {
  ...
  */
  async traveltime(time) {
    ...
    while (safetyCounter && (targetTime - this.totalMoveTime > 1e-4)) {
      if (this._stopped) {
        throw new Error("traveltime() manually stopped");
      }
      safetyCounter--;
